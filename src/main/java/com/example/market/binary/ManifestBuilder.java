...
//1) почему мультипарт в бинарном теле, а не заголовок
//2) что такое офсеты (sig.getOffset)
//3) что такое magic
//4) порядок следлования байт в битовом представлении
//5) что в манифесте подписывается, а что нет, куда кладется подпись